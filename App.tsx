...
// Countdown timer for rewarded offers (1 second tick). Don't remove an offer at 0s while its popup is open. Pause while fake ad is visible.
  ...
  useEffect(() => {
    ...
    rafId = requestAnimationFrame(tick);
    return () => cancelAnimationFrame(rafId);
  }, [seedProductionLevel, isLoading, activeBoosts, activeFtueStage, goldenPotCount]);
  ...
  useEffect(() => {
    ...
    if (effectiveGoalLoadingSeconds <= 0) {
      ...
      applyDiscoveryRemainingAfterSpawn(
        discoverySpawnCommitSeq,
        plantLevel,
        highestPlantEverRef,
        highestPlantEverStateRef,
        discoveryGoalsRemainingRef
      );
      const patchedTypes = [...(goalPlantTypesRef.current ?? types)];
      ...
      setGoalLoadingSeconds((prev) => {
        if (prev <= 1) {
          ...
          try {
            ...
            recordSpawnedGoalPlantLevel(plantLevel, lastSpawnedGoalLevelsRef, lastSpawnedGoalPlantLevelHUDRef);
            applyDiscoveryRemainingAfterSpawn(
              discoveryCommitSeqForThisIntervalTick!,
              plantLevel,
              highestPlantEverRef,
              highestPlantEverStateRef,
              discoveryGoalsRemainingRef
            ...
            setTimeout(() => {
              ...
              const maxSlots = getMaxGoalSlots(playerLevel);
              const firstEmptyIdx = goalSlots.findIndex((s, i) => s === 'empty' && i < maxSlots);
              setGoalBounceSlots((p) => p.filter((s) => s !== loadingIdx));
              ...
              if (firstEmptyIdx >= 0) {
                setGoalDisplayOrder((p) => (p.includes(firstEmptyIdx) ? p : [...p, firstEmptyIdx]));
                setGoalSlotFadeInSlot(firstEmptyIdx);
                setGoalLoadingSeconds(hasRushOrdersBoost ? 0 : getGoalLoadingSeconds(harvestState, goldenPotCount));
                setTimeout(() => setGoalSlotFadeInSlot(null), 500);
              }
  ...
  useEffect(() => {
    ...
    rafId = scheduleNextFrame(tick);
    return () => cancelAnimationFrame(rafId);
  }, [harvestSpeedLevel, isLoading, activeBoosts, activeFtueStage, ftue7Scheduled, ftueHarvestSurplusActivated, ftueSeedSurplusActivated, seedsState, goldenPotCount]);
  ...
  const performMergeHarvest = useCallback((centerCellIdx: number, chancePercent: number, excludeCellIdx?: number) => {
    ...
    triggeredCells.forEach((cellIdx) => {
      ...
      if (hasGoalForPlant) {
        ...
        if (hasGoldenPotMergeCoinsDouble(goldenPotCount)) value *= 2;
  ...
  const hydrateFromSave = useCallback((save: GameSaveV1) => {
    ...
    const goldPotsForOffline = save.plantMasteryUnlockedLevels?.length ?? 0;
    ...
    let totalOffline = pendingBank + sim.offlineSurplusCoins;
    if (hasGoldenPotOfflineEarningsDouble(goldPotsForOffline)) totalOffline *= 2;
            ...
            <div className="w-1/3 h-full flex flex-col relative overflow-hidden">
              ...
              >
                ...
                >
                  ...
                  <div className="relative flex flex-col items-center" style={{ marginTop: 215 }} data-barn-shelves>
